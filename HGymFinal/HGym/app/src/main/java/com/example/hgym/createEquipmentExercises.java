...
//intent for creating equipment exercises.
public class createEquipmentExercises extends AppCompatActivity implements View.OnClickListener {

    EditText emailEdit ,nameE, muscleE, levelE, expE, weightE;
    Button btnSave, btnReturn, btnDelete, btnUpdate;
    String name, muscle, level, exp, weight;
...
//                });
                        ...
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            if (task.isSuccessful()) {
                                for (QueryDocumentSnapshot document : task.getResult()) {
                                    ...
                                    ArrayList<equipmentExercise> eq = data.getEquipmentExercise();
                                    ...
                                    equipmentExercise exercise = new equipmentExercise(muscle, name, level, exp, Double.parseDouble(weight));
                                    boolean flag = eq.add(exercise);
                                    DocumentReference docRef = db.collection("users").document(document.getId());
                                    docRef.update("equipmentExercise", eq)
                        ...
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            if (task.isSuccessful()) {
                                for (QueryDocumentSnapshot document : task.getResult()) {
                                    ...
                                    weight = weightE.getText().toString();
                                    equipmentExercise exercise = new equipmentExercise(muscle, name, level, exp, Double.parseDouble(weight));
                                    ArrayList<equipmentExercise> neqEq = new ArrayList<>();
                                    ...
                                    DocumentReference docRef = db.collection("users").document(document.getId());
                                    docRef.update("equipmentExercises", neqEq)
                                            .addOnCompleteListener(new OnCompleteListener<Void>() {
                                                @Override
                                                public void onComplete(@NonNull Task<Void> task) {
                                                    if(task.isSuccessful()){
                        ...
                        public void onComplete(@NonNull Task<QuerySnapshot> task) {
                            if (task.isSuccessful()) {
                                for (QueryDocumentSnapshot document : task.getResult()) {
                                    Log.d("s", document.getId() + " => " + document.getData());
                                    user data = document.toObject(user.class);
                                    ArrayList<equipmentExercise> exercises = data.getEquipmentExercise();
                                    ArrayList<equipmentExercise> newArr = new ArrayList<>();
                                    for(int i=0;i<exercises.size();i++){
                                        ...
                                        if(name.equals(exercises.get(i).getName())){
                                            ...
                                            db.collection("users").document(mAuth.getCurrentUser().getEmail())
                                                    .update("equipmentExercise", newArr).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        @Override
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            if(task.isSuccessful()){
                                                                Log.d("TAG","good");
                                                                Toast.makeText(createEquipmentExercises.this, "deleting succeeded", Toast.LENGTH_SHORT).show();
                                                            } else {
                                                                Log.d("TAG", "bad");
                                                                Toast.makeText(createEquipmentExercises.this, "deleting failed", Toast.LENGTH_SHORT).show();
                                                            }
                                                        }
                                                    });
                                        } else {
    ...
    public boolean eqEx(equipmentExercise ex1, equipmentExercise ex2){
        if(ex1.getWeight() == ex2.getWeight()){
